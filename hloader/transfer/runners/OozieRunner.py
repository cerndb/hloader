...
<configuration>
    ...
    <property>
        ...
        response = requests.post(urllib.parse.urljoin(self.oozie_base_URL, '/oozie/v2/jobs?jobtype=sqoop'),
                                 data=workflow_xml, headers=headers)
        if response.status_code == 200:
            return response.json()['id']

    def manage(self, job: Job, action: str):
        if action not in ('start', 'suspend', 'resume', 'kill', 'dryrun'):
            print("Unsupported action")

        URL = urllib.parse.urljoin(self.oozie_base_URL,
    ...
    def job_info(self, job: Job, timezone: str = "CET"):
        URL = urllib.parse.urljoin(self.oozie_base_URL,
